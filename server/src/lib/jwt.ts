...
class JWTService {
   ...
   */
  async signServerJWT(payload: { sub: string; provider: 'google' | 'apple' }): Promise<string> {
    try {
   ...
   */
  async verifyServerJWT(token: string): Promise<ServerJWTPayload> {
    try {
      const privateKey = await this.getPrivateKey();
      ...
      return payload as ServerJWTPayload;
    } catch (error) {
   ...
   */
  extractUserContext(payload: ServerJWTPayload): UserContext {
    return {
      userId: payload.sub,
      provider: payload.provider,
    };
   ...
   * Get expiration info
   */
  getExpirationInfo(): { expiresIn: number } {
  ...
  private async getPublicKeyFromPrivate(privateKey: CryptoKey): Promise<CryptoKey> {
    try {
  ...
  private async cryptoKeyToJWK(key: CryptoKey): Promise<JWK> {
    return await crypto.subtle.exportKey('jwk', key) as JWK;
  }
}
